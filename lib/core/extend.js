...
async function Extend(sock, options = {}) {
  ...
  sock.sendAlbum = async function (jid, items, quoted, options = {}) {
    // Allow: sendAlbum(jid, items, options) without quoted
    if (quoted && typeof quoted === 'object' && !quoted.key && !quoted.message && options === undefined) {
      options = quoted
    ...
    for (const item of items) {
      ...
      let forceVideo = !!(item.video || item.isVideo)
      ...
      if (isVideo) {
        array.push({ video: mediaValue, caption, mimetype: mime || 'video/mp4' })
      } else {
