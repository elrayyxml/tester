...
async function Extend(sock, options = {}) {
  ...
  // ===================== ALBUM =====================
  // Pattern from messages-send.js: header albumMessage then child media
  // with messageContextInfo.messageAssociation = { parentMessageKey, associationType: MEDIA_ALBUM }
  sock.sendAlbum = async function (jid, items, quoted, options = {}) {
    ...
    for (const item of items) {
      const buffer = await getBufferFromUrl(item.buffer || item.url || item.image || item.video || item)
    ...
    // 2) children with association + delay
    ...
    for (const item of prepared) {
      ...
      const childContent = createMessage({
        ...preparedMedia,
        messageContextInfo: {
          messageAssociation: {
            associationType: 1, // MEDIA_ALBUM / AssociationType.MEDIA_ALBUM
