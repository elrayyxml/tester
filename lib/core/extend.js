...
const { getBufferFromUrl, detectMime, decodeAudio } = require('../utils/media')
...
async function Extend(sock, options = {}) {
  ...
  sock.sendAudio = async function (jid, bufferOrUrl, quoted, options = {}) {
    if (!isQuotedLike(quoted) && isOptionsObject(quoted) && arguments.length < 4) {
      options = quoted
      quoted = options.quoted
    }
    const ptt = options.ptt === true
    const decoded = await decodeAudio(bufferOrUrl, {
      ptt,
      ...
      sampleCount: options.waveformSamples
    })
    return relayHelper(sock, jid, {
      audio: decoded.audio,
      mimetype: options.mimetype || options.mime || 'audio/ogg; codecs=opus',
      ptt,
  ...
  sock.sendMedia = async function (jid, bufferOrUrl, caption, quoted, options = {}) {
    ...
    return sock.sendAudio(jid, buffer, quoted, options)
  }
  ...
  sock.sendAlbum = async function (jid, items, quoted, options = {}) {
    ...
    for (const item of items) {
      // Support: { image }, { video }, { media }, { url }, buffer, path, string url
      let src = item.image || item.video || item.media || item.buffer || item.url || item
    ...
    for (const contentItem of array) {
      ...
      await sock.relayMessage(jid, mediaMsg.message, {
        ...
        // quoted parent so association is visible
