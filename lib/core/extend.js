...
async function Extend(sock, options = {}) {
  ...
  sock.sendText = async function (jid, text, quoted, options = {}) {
    ...
    if (options.mentionAll) content.mentions = options.mentions // caller should expand
  ...
  // ---------- sendAlbum ----------
  sock.sendAlbum = async function (jid, items, quoted) {
    // Album is sent as sequential media with association – simplified here
    const results = []
    for (const item of items) {
      const r = await sock.sendMedia(jid, item.buffer || item.url, item.caption, quoted, item)
      results.push(r)
    }
    return results
  }
  ...
  // ---------- Interactive (covers nativeFlow + buttons + list sections) ----------
  sock.sendInteractive = async function (jid, buttons, quoted, options = {}) {
    // Support both classic buttons and nativeFlow
    const content = {
      interactiveButtons: buttons,
      ...options
    }
    if (options.media) {
      // media can be image/video/document – handled by generateWAMessage
      Object.assign(content, options.media)
    }
    if (options.caption) content.caption = options.caption
    if (options.footer) content.footer = options.footer
