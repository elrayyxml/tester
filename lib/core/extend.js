...
const { getBufferFromUrl, detectMime, prepareAudioForSend } = require('../utils/media')
...
async function Extend(sock, options = {}) {
  ...
  // ===================== MEDIA =====================
  sock.sendImage = async function (jid, bufferOrUrl, caption, quoted, options = {}) {
    if (isOptionsObject(caption) && quoted === undefined && arguments.length === 3) {
      options = caption
      caption = options.caption
      quoted = options.quoted
    } else if (!isQuotedLike(quoted) && isOptionsObject(quoted) && arguments.length < 5) {
      options = quoted
      quoted = options.quoted
    }
    const buffer = await getBufferFromUrl(bufferOrUrl)
    return relayHelper(sock, jid, {
      image: buffer,
      caption: caption ?? options.caption,
      mimetype: options.mimetype || options.mime,
      jpegThumbnail: options.jpegThumbnail || options.thumbnail,
      viewOnce: options.viewOnce,
      ...options.mediaOptions
    }, quoted, genOpts(options))
  }

  sock.sendVideo = async function (jid, bufferOrUrl, caption, quoted, options = {}) {
    if (isOptionsObject(caption) && quoted === undefined && arguments.length === 3) {
      options = caption
      caption = options.caption
      quoted = options.quoted
    ...
    return relayHelper(sock, jid, {
      ...
      gifPlayback: options.gifPlayback === true || options.gif === true,
      viewOnce: options.viewOnce,
      ...options.mediaOptions
    }, quoted, genOpts(options))
  }

  sock.sendAudio = async function (jid, bufferOrUrl, captionOrQuoted, quotedOrOptions, options = {}) {
    let caption
    let quoted
    let sendOptions
    if (isQuotedLike(quotedOrOptions)) {
      ...
      sendOptions = options || {}
    } else if (isQuotedLike(captionOrQuoted)) {
      quoted = captionOrQuoted
      sendOptions = quotedOrOptions && isOptionsObject(quotedOrOptions) ? quotedOrOptions : options
    } else if (isOptionsObject(captionOrQuoted)) {
      quoted = captionOrQuoted.quoted
      sendOptions = captionOrQuoted
    } else {
      caption = captionOrQuoted
      quoted = quotedOrOptions && isQuotedLike(quotedOrOptions) ? quotedOrOptions : undefined
      sendOptions = quotedOrOptions && isOptionsObject(quotedOrOptions) ? quotedOrOptions : options
    }
    sendOptions = sendOptions || {}
    const ptt = sendOptions.ptt === true
    const decoded = await prepareAudioForSend(bufferOrUrl, {
      ptt,
      transcode: sendOptions.transcode,
      waveform: sendOptions.waveform,
      seconds: sendOptions.seconds,
      logger: opts.logger,
      ffmpegPath: sendOptions.ffmpegPath,
      ffprobePath: sendOptions.ffprobePath,
      bitrate: sendOptions.bitrate
    })
    return relayHelper(sock, jid, {
      audio: decoded.audio,
      caption: caption ?? sendOptions.caption,
      mimetype: ptt ? 'audio/ogg; codecs=opus' : (sendOptions.mimetype || sendOptions.mime || 'audio/mpeg'),
      ptt,
      waveform: decoded.waveform,
      seconds: decoded.seconds,
      backgroundArgb: sendOptions.backgroundArgb,
      viewOnce: sendOptions.viewOnce,
      ...sendOptions.mediaOptions
    }, quoted, genOpts(sendOptions))
  }

  sock.sendMedia = async function (jid, bufferOrUrl, caption, quoted, options = {}) {
    if (!isQuotedLike(quoted) && isOptionsObject(quoted) && arguments.length < 5) {
      options = quoted
      quoted = options.quoted
    }
    const buffer = await getBufferFromUrl(bufferOrUrl)
    ...
    if (mime.startsWith('image/')) return sock.sendImage(jid, buffer, caption, quoted, options)
    if (mime.startsWith('video/')) return sock.sendVideo(jid, buffer, caption, quoted, options)
    return sock.sendAudio(jid, buffer, caption, quoted, options)
  }

  sock.sendPtv = async function (jid, bufferOrUrl, quoted, options = {}) {
    return sock.sendVideo(jid, bufferOrUrl, undefined, quoted, { ...options, ptv: true })
  }
  ...
  sock.sendLocation = async function (jid, loc, quoted, options = {}) {
    if (!isQuotedLike(quoted) && isOptionsObject(quoted) && arguments.length < 4) {
      options = quoted
      quoted = options.quoted
    }
    const location = {
      latitude: loc?.latitude ?? loc?.degreesLatitude ?? 0,
      longitude: loc?.longitude ?? loc?.degreesLongitude ?? 0,
      name: loc?.name || '',
      address: loc?.address || '',
      url: loc?.url || '',
      jpegThumbnail: loc?.jpegThumbnail || loc?.thumbnail
    }
    const buttons = options.buttons || options.interactiveButtons || options.nativeFlow
    if (Array.isArray(buttons) && buttons.length) {
      return sock.sendInteractive(jid, buttons, quoted, {
        ...options,
    ...
    return relayRaw(
      sock,
      jid,
      buildLocationContent({ ...location, mentions: options.mentions, mentionAll: options.mentionAll, contextInfo: options.contextInfo }),
      { ...genOpts(options), quoted, contextInfo: options.contextInfo }
    )
  }

  // ===================== ALBUM =====================
  // handleAlbum pattern: header (messageSecret + counts) then children with association
  sock.sendAlbum = async function (jid, items, quoted, options = {}) {
    // Supported forms: sendAlbum(jid, items, quoted, options) or sendAlbum(jid, items, options).
    if (isOptionsObject(quoted) && arguments.length < 4) {
      options = quoted
      quoted = options.quoted
    }
    options = options || {}
    ...
    for (const rawItem of items) {
      const item = Buffer.isBuffer(rawItem) || typeof rawItem === 'string' ? { media: rawItem } : (rawItem || {})
      // Support upstream-style { image }, { video }, { media }, { url }, Buffer, path, and URL.
      let src = item.image || item.video || item.media || item.buffer || item.url
      let caption = item.caption
      let forceVideo = !!(item.video || item.isVideo || item.type === 'video')

      // Resolve buffer for mime detection when possible
      let buffer = null
      let pathOrUrl = null
      if (Buffer.isBuffer(src)) {
        ...
        pathOrUrl = src
        if (/^https?:\/\//i.test(src)) {
          try { buffer = await getBufferFromUrl(src) } catch {}
        }
      } else if (src && typeof src === 'object' && src.url) {
        pathOrUrl = src.url
        try { buffer = await getBufferFromUrl(src.url) } catch {}
      }

      let mime = item.mimetype || item.mime || null
      if (!mime && buffer) {
        try { mime = await detectMime(buffer) } catch {}
      }
      if (!mime && pathOrUrl) {
        const lower = pathOrUrl.toLowerCase()
        if (/\.(mp4|mkv|mov|webm|3gp)(\?|$)/.test(lower)) mime = 'video/mp4'
        else if (/\.(jpg|jpeg|png|webp|gif)(\?|$)/.test(lower)) mime = 'image/jpeg'
      }

      const isVideo = forceVideo || (mime && mime.startsWith('video/'))
      if (!src) {
        throw new NexrayError('Album item must contain image, video, media, url, buffer, or path', { code: 'INVALID_ALBUM_ITEM' })
      }
      const mediaValue = buffer || (pathOrUrl ? { url: pathOrUrl } : src)
      ...
      if (isVideo) {
        array.push({
          ...
          gifPlayback: item.gifPlayback === true || item.gif === true,
          ptv: item.ptv === true
        })
      } else {
        array.push({ image: mediaValue, caption, mimetype: mime || 'image/jpeg' })
      }
    }

    const imageCount = array.filter(a => Object.prototype.hasOwnProperty.call(a, 'image')).length
    const videoCount = array.filter(a => Object.prototype.hasOwnProperty.call(a, 'video')).length

    if (imageCount + videoCount < 2) {
      throw new NexrayError('Album requires at least 2 valid media items', { code: 'INVALID_ALBUM' })
    }

    // The upstream fork implements album child relay inside sendMessage({ album }).
    // Prefer it when detected; keep the manual path below for stock Baileys.
    const nativeAlbumSupported = options.nativeAlbum === true ||
      (options.nativeAlbum !== false && typeof sock.sendMessage === 'function' && /album/.test(Function.prototype.toString.call(sock.sendMessage)))
    if (nativeAlbumSupported) {
      return sock.sendMessage(jid, { album: array }, {
        ...options,
        quoted,
        delayMs,
        messageId: undefined
    ...
    const genOptsLocal = {
      ...
      quoted: quoted?.key ? { key: quoted.key, message: quoted.message } : quoted,
      upload: sock.waUploadToServer,
      mediaCache: sock._nexrayMediaCache,
      messageId: generateMessageId(sock.user?.id, opts.messageIdPrefix),
      logger: opts.logger
    }

    const msg = await generateWAMessageFromContent(jid, albumContent, genOptsLocal)
    ...
    for (const contentItem of array) {
      ...
      const normalizedMedia = require('../utils/message').normalizeMessageContent(mediaMsg.message)
      if (!normalizedMedia?.imageMessage && !normalizedMedia?.videoMessage) {
        throw new NexrayError('Invalid message type for album child', { code: 'INVALID_ALBUM_MEDIA' })
      }
  ...
  // Product list follows the source mapping in messages.md.
  sock.sendProductList = async function (jid, productList, quoted, options = {}) {
    if (isOptionsObject(quoted) && arguments.length < 4) {
      options = quoted
      quoted = options.quoted
    }
    if (!Array.isArray(productList) || !productList.length) {
      throw new NexrayError('productList must be a non-empty array', { code: 'INVALID_PRODUCT_LIST' })
    }
    if (!options.businessOwnerJid) {
      throw new NexrayError('businessOwnerJid is required for productList', { code: 'MISSING_BUSINESS_OWNER' })
    }
    const firstProduct = productList[0]?.products?.[0]
    if (!firstProduct?.productId) {
      throw new NexrayError('First product must have a valid productId', { code: 'INVALID_PRODUCT_ID' })
    }
    let jpegThumbnail
    if (options.thumbnail) {
      const prepared = await prepareMedia(sock, { image: await getBufferFromUrl(options.thumbnail) }, genOpts(options))
    ...
    const content = createMessage({
      listMessage: {
        ...
        description: options.text || options.caption || '',
        productListInfo: {
          productSections: productList,
          headerImage: { productId: firstProduct.productId, jpegThumbnail },
          businessOwnerJid: options.businessOwnerJid
        },
        listType: options.listType ?? 1
      }
    })
    return relayRaw(sock, jid, content, { ...genOpts(options), quoted, contextInfo: options.contextInfo })
  }

  // Product + optional interactive buttons. Shape follows upstream prepareProductMessage.
  sock.sendProduct = async function (jid, options = {}, quoted) {
    if (options.productList) {
      return sock.sendProductList(jid, options.productList, quoted, options)
    }
    const source = options.product && typeof options.product === 'object' ? options.product : options
    const businessOwnerJid = options.businessOwnerJid || source.businessOwnerJid
    if (!businessOwnerJid) {
      throw new NexrayError('businessOwnerJid is required for product message', { code: 'MISSING_BUSINESS_OWNER' })
    }
    const img = options.image || options.productImage || options.buffer || source.productImage
    if (!img) {
      throw new NexrayError('productImage is required', { code: 'MISSING_PRODUCT_IMAGE' })
    }
    const product = {
      ...source,
      productId: options.productId ?? source.productId ?? '',
      title: options.title ?? source.title ?? '',
      description: options.description ?? source.description ?? '',
      currencyCode: options.currencyCode ?? source.currencyCode ?? 'IDR',
      priceAmount1000: options.priceAmount1000 ?? source.priceAmount1000 ?? 0,
      productImageCount: options.productImageCount ?? source.productImageCount ?? 1,
      retailerId: options.retailerId ?? source.retailerId,
      url: options.url ?? source.url
    }

    const buttons = options.interactiveButtons || options.buttons
    if (Array.isArray(buttons) && buttons.length) {
      return sock.sendInteractive(jid, buttons, quoted, {
        ...options,
        text: options.caption || options.text,
        product: { ...product, productImage: img, businessOwnerJid },
        businessOwnerJid,
  ...
  sock.sendLegacyButton = async function (jid, buttons, quoted, options = {}) {
    ...
    // image / video header
    if (options.image || options.video || (options.media && !options.media.location)) {
    ...
    // location header (business-style location + buttons)
    if (options.location || options.media?.location) {
      const loc = options.location || options.media.location
      headerMedia = {
        locationMessage: {
          degreesLatitude: loc.degreesLatitude ?? loc.latitude ?? 0,
          degreesLongitude: loc.degreesLongitude ?? loc.longitude ?? 0,
...
function isQuotedLike(value) {
  return !!(value && typeof value === 'object' && value.key && (value.message || value.fakeObj || value.messageTimestamp != null))
}

function isOptionsObject(value) {
  return !!(value && typeof value === 'object' && !Buffer.isBuffer(value) && !isQuotedLike(value))
}
