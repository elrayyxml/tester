...
async function Extend(sock, options = {}) {
  ...
  // ===================== ALBUM =====================
  // Exact pattern: header with messageSecret + albumMessage, then children
  // with messageAssociation + quoted pointing to parent
  sock.sendAlbum = async function (jid, items, quoted, options = {}) {
    ...
    // Normalize items to { image|video: buffer/url, caption? }
    const array = []
    for (const item of items) {
      ...
      const src = item.buffer || item.url || item
      const buffer = typeof src === 'string' && !/^https?:/i.test(src)
        ? src // path – let generateWAMessage handle
        : await getBufferFromUrl(src).catch(() => src)
      let mime = item.mimetype || item.mime
      if (Buffer.isBuffer(buffer) && !mime) mime = await detectMime(buffer)
      const isVideo = (mime && mime.startsWith('video/')) || !!item.video
      if (isVideo) array.push({ video: buffer, caption: item.caption, mimetype: mime })
      else array.push({ image: buffer, caption: item.caption, mimetype: mime })
    }
