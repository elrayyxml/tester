...
export function createMessageApi(ctx) {
    ...
    async function sendBuilt(remoteJid, fullMsg, messageId, opts, extra = {}) {
        await caps.relayMessage(
            remoteJid,
            fullMsg.message,
            relayOpts(
    ...
    async function sendEvent(remoteJid, event, quoted = null, options = {}) {
        ...
        const messageId = await nextId(opts);
        const eventNode = eventCreationNode();
        const genOpts = {
            userJid: ctx.meId,
            messageId,
            quoted: quoted || undefined,
        ...
        // 1) Native messages.md path: { event }
        try {
            ...
            if (fullMsg?.message?.eventMessage) {
                return sendBuilt(remoteJid, fullMsg, messageId, opts, {
                    event: true,
                    additionalNodes: [eventNode]
                });
            }
        } catch {
            /* fall through to FromContent */
        }

        // 2) Fallback: eventMessage FromContent (messages.md shape)
        const startTime = Math.floor(startDate.getTime() / 1000);
        const eventMessage = {
            name: event.name,
            description: event.description,
            startTime,
            endTime: endDate && !Number.isNaN(endDate.getTime()) ? endDate.getTime() / 1000 : undefined,
    ...
    async function sendPoll(remoteJid, values, pollOptions = {}, quoted = null, options = {}) {
        ...
        const polltype = isQuiz ? 'quiz_creation' : 'creation';
        const pollNode = pollMetaNode(polltype, isNewsletter ? { contenttype: 'text' } : {});
        const genOpts = {
            userJid: ctx.meId,
            messageId,
            quoted: quoted || undefined
        ...
        // 1) Native messages.md path: { poll }
        try {
    ...
    async function sendPollResult(remoteJid, name, votes, quoted = null, options = {}) {
        ...
        const messageId = await nextId(opts);
        const pollNode = pollMetaNode('creation');
        const genOpts = {
            userJid: ctx.meId,
            messageId,
            quoted: quoted || undefined
