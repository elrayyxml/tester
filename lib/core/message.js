...
export function createMessageApi(ctx) {
    ...
    /**
     * Event (messages.md: content.event → eventMessage + meta event_type).
    ...
    async function sendEvent(remoteJid, event, quoted = null, options = {}) {
        ...
        const eventPayload = {
            ...
            startDate,
            endDate: event.endDate != null ? (event.endDate instanceof Date ? event.endDate : new Date(event.endDate)) : undefined,
        ...
        // messages.md path: generateWAMessageContent({ event })
        ...
        // messages-send.md: meta event_type=creation
        await caps.relayMessage(
            remoteJid,
            fullMsg.message,
            relayOpts(
                ctx,
                {
                    ...
                    event: true,
                    additionalNodes: [...(opts.additionalNodes || []), eventCreationNode()]
                },
                fullMsg.message
            )
        );
    ...
    /**
     * Poll (messages.md: content.poll → pollCreationMessage* + meta polltype).
    ...
    async function sendPoll(remoteJid, values, pollOptions = {}, quoted = null, options = {}) {
        ...
        // messages.md path: generateWAMessageContent({ poll })
        const fullMsg = await caps.generateWAMessage(
            remoteJid,
            { poll },
            {
        ...
        // messages-send.md: meta polltype (+ contenttype for newsletter)
        const isNewsletter = String(remoteJid).includes('@newsletter');
        const isQuiz = poll.pollType === 1;
    ...
    /**
     * Poll result (messages.md: content.pollResult).
