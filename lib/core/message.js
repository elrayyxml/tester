...
import { buildAdditionalNodes, pollMetaNode, eventCreationNode } from './node.js';
...
export function createMessageApi(ctx) {
    ...
    /**
     * Contact / business contact (vCard).
     * Single object, array, or contacts with biz_name / biz_description.
     */
    async function sendContact(remoteJid, contactOrList, quoted = null, options = {}) {
        ...
        try {
            const content = buildContactMessage(contactOrList);
            const messageId =
                opts.messageId ||
                generateMessageId({ meId: ctx.meId, customId: config.customId, stealth: config.stealth });
            ...
            if (fullMsg?.message && (opts.mentions || opts.contextInfo)) {
                fullMsg.message = applyContextInfo(fullMsg.message, opts);
            }
    ...
    /** Detect poll / event from built message → meta nodes (messages-send.md). */
    function metaNodesFor(message, remoteJid, opts = {}) {
        ...
        if (isPoll) {
            const isQuiz =
                !!m.pollCreationMessageV5 ||
                m.pollResultSnapshotMessageV3?.pollType === 1 ||
                opts.polltype === 'quiz_creation';
            nodes.push(
                pollMetaNode(isQuiz ? 'quiz_creation' : 'creation', isNewsletter ? { contenttype: 'text' } : {})
            );
        }
    ...
    /**
     * Single relay entry for every helper.
     * Applies metaLabel (via relayOpts) + auto event/poll meta nodes.
     */
    async function dispatch(remoteJid, fullMsg, opts = {}) {
        if (!fullMsg?.message) {
            throw createError(Err.INVALID_MESSAGE, 'Empty message');
        }

        // floor event times (WA rejects float endTime)
        const em = fullMsg.message.eventMessage;
        if (em) {
            if (em.startTime != null) em.startTime = Math.floor(Number(em.startTime));
            if (em.endTime != null) em.endTime = Math.floor(Number(em.endTime));
        }

        const messageId = fullMsg.key?.id || nextId(opts);
        if (fullMsg.key) fullMsg.key.id = messageId;

        const nodes = metaNodesFor(fullMsg.message, remoteJid, opts);
        const isEvent = !!fullMsg.message.eventMessage;
        const isPoll = nodes.some((n) => n.attrs?.polltype);
        ...
        await caps.relayMessage(
            ...
            relayOpts(
                ctx,
                {
                    ...opts,
                    messageId,
                    event: isEvent || opts.event,
                    polltype: isPoll ? opts.polltype || 'creation' : opts.polltype,
    ...
    /**
     * Event — native { event } if engine supports, else eventMessage FromContent.
     * sock.sendEvent(remoteJid, event, quoted?)
     */
    async function sendEvent(remoteJid, event, quoted = null, options = {}) {
        ...
        const genOpts = {
            ...
            messageId,
            quoted: quoted || undefined,
            getCallLink: opts.getCallLink || sock.getCallLink
        };
        ...
        try {
            ...
            if (fullMsg?.message?.eventMessage) {
                return dispatch(remoteJid, fullMsg, opts);
            }
        } catch {
        ...
        if (event.call) {
            const getLink = opts.getCallLink || sock.getCallLink;
            if (typeof getLink === 'function') {
                const token = await getLink(event.call, { startTime });
                const prefix =
                    event.call === 'audio'
                        ? 'https://call.whatsapp.com/voice/'
                        : 'https://call.whatsapp.com/video/';
                eventMessage.joinLink = prefix + token;
            }
        }
    ...
    /**
     * Poll — native { poll } if engine supports, else pollCreationMessage* FromContent.
     * sock.sendPoll(remoteJid, values, { name, selectableCount }, quoted?)
     */
    async function sendPoll(remoteJid, values, pollOptions = {}, quoted = null, options = {}) {
        ...
        try {
            const fullMsg = await caps.generateWAMessage(remoteJid, { poll }, genOpts);
            ...
            if (
                ...
                return dispatch(remoteJid, fullMsg, {
                    ...opts,
        ...
        if (poll.toAnnouncementGroup) {
            content = { pollCreationMessageV2: pollCreationMessage };
        } else if (isQuiz) {
            ...
            content = { pollCreationMessageV3: pollCreationMessage };
        } else {
            content = { pollCreationMessage };
        }
        content.messageContextInfo = { messageSecret };

        const fullMsg = await caps.generateWAMessageFromContent(remoteJid, content, genOpts);
        return dispatch(remoteJid, fullMsg, {
            ...opts,
            polltype: isQuiz ? 'quiz_creation' : 'creation'
        });
    ...
    /**
     * Poll result — native { pollResult } or FromContent.
     * sock.sendPollResult(remoteJid, name, votes, quoted?)
     */
    async function sendPollResult(remoteJid, name, votes, quoted = null, options = {}) {
        ...
        try {
            const fullMsg = await caps.generateWAMessage(remoteJid, { pollResult }, genOpts);
            if (
                fullMsg?.message?.pollResultSnapshotMessage ||
                fullMsg?.message?.pollResultSnapshotMessageV3
            ) {
                return dispatch(remoteJid, fullMsg, { ...opts, polltype: 'creation' });
            }
        } catch {
            /* fall through */
        }
        ...
        let content;
        if (opts.pollType === 1) {
            snap.pollType = caps.proto?.Message?.PollType?.QUIZ ?? 1;
            content = { pollResultSnapshotMessageV3: snap };
        } else {
            snap.pollType = caps.proto?.Message?.PollType?.POLL ?? 0;
            content = { pollResultSnapshotMessage: snap };
        }
