...
export function createMessageApi(ctx) {
    ...
    /**
     * Send a sticker pack.
     * Builds stickerPackMessage then relays via generateWAMessageFromContent.
     */
    async function sendStickerPack(remoteJid, pack, quoted = null, options = {}) {
        ...
        if (typeof caps.prepareStickerPackMessage === 'function') {
            stickerPackMessage = await caps.prepareStickerPackMessage(
                {
                    stickers: stickers.map((entry) => ({
                        data: toMediaSource(entry.sticker),
                        emojis: entry.emojis,
            ...
            try {
                ...
                throw createError(Err.INVALID_MEDIA, error?.message || String(error));
            }
        }
