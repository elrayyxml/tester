...
export function createMessageApi(ctx) {
    ...
    /**
     * Send a sticker pack (Baileys prepareStickerPackMessage via stickers content key).
     * Signature: sendStickerPack(jid, pack, quoted?)
     *
     ...
     *   description?: string,
     *   emojis?: string[],
     *   cover: string|Buffer|object,
     *   stickers: Array<{ sticker: string|Buffer|object, emojis?: string[], accessibilityLabel?: string }>
     * }} pack
    ...
    async function sendStickerPack(remoteJid, pack, quoted = null, options = {}) {
        ...
        const stickers = pack.stickers.map((item, index) => {
            ...
            if (source == null || (typeof source === 'object' && !Buffer.isBuffer(source) && source.url == null && source.stream == null && typeof source !== 'string')) {
                throw createError(Err.INVALID_MEDIA, `Invalid sticker at index ${index}`);
            }
            return {
                data: toMediaSource(source),
        ...
        // messages.md: generateWAMessageContent uses property "stickers" + cover/name/publisher/description
        const content = {
            stickers,
