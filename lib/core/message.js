...
export function createMessageApi(ctx) {
    ...
    /**
     * Carousel cards (messages.md: content.cards → interactiveMessage.carouselMessage).
     * Card media: image|video as Buffer|path|url
     * Buttons: card.buttons or card.nativeFlow → nativeFlowMessage
     */
    async function sendCard(remoteJid, pack, quoted = null, options = {}) {
        const opts = options || {};
        if (!pack || typeof pack !== 'object') {
            throw createError(Err.INVALID_MESSAGE, 'Card payload is required');
        }
        ...
        const cards = pack.cards.map((card, index) => {
            if (!card || typeof card !== 'object') {
                throw createError(Err.INVALID_MESSAGE, `Invalid card at index ${index}`);
            }
            if (card.image == null && card.video == null && card.product == null) {
                throw createError(Err.INVALID_MEDIA, `Card ${index} needs image, video, or product`);
            }
            ...
            // nativeFlow buttons (cta_url, quick_reply, etc.)
        ...
        if (opts.messageParamsJson || pack.messageParamsJson) {
            content.messageParamsJson = opts.messageParamsJson || pack.messageParamsJson;
        }

        const additionalNodes = [
            ...(opts.additionalNodes || []),
            interactiveBizNode('mixed')
        ];
    ...
    /**
     * Interactive message (messages.md: interactiveButtons → nativeFlowMessage).
     *
     * await sock.sendInteractive(jid, {
     *   image|video, text|caption, footer, title,
     *   mentions, interactiveButtons|buttons
     * }, quoted?)
     */
    async function sendInteractive(remoteJid, payload, quoted = null, options = {}) {
        ...
        return relayMessagePipeline(ctx, remoteJid, content, {
            ...
            additionalNodes: [
                ...(opts.additionalNodes || []),
                interactiveBizNode(opts.nativeFlowName || 'mixed')
            ]
        });
    }
