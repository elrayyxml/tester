...
export function createMessageApi(ctx) {
    ...
    /**
     * Cards → interactiveMessage.carouselMessage (messages.md)
     */
    async function sendCard(remoteJid, pack, quoted = null, options = {}) {
        ...
        try {
            ...
            if (pack.mentions || opts.mentions) {
                opts.mentions = pack.mentions || opts.mentions;
            }
     ...
     * sock.sendInteractive(remoteJid, payload, quoted?)
     */
    async function sendInteractive(remoteJid, payload, quoted = null, options = {}) {
        ...
        try {
            const content = await buildInteractiveMessage(
    ...
    /**
     * Product (messages.md product → productMessage / interactive header)
     * sock.sendProduct(remoteJid, payload, quoted?)
     */
    async function sendProduct(remoteJid, payload, quoted = null, options = {}) {
        ...
        try {
            ...
            if (buttons && (Array.isArray(buttons) ? buttons.length : true)) {
                // messages.md: product as interactive header + nativeFlow
