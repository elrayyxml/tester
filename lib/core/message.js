...
export function generateStealthId(device, prefix = '') {
    ...
    switch (device) {
        case 'ios':
            return p + '3A' + getRandom(18)
        case 'web':
            return p + '3E' + getRandom(20)
        case 'android':
            return p + getRandom(32)
        case 'desktop':
            return p + '3F' + getRandom(18)
        default:
            return p + getRandom(36).toLowerCase()
    }
}
...
export async function sendInteractive(sock, remoteJid, payloadOrButtons, quotedOrOptions, maybeOptions) {
    ...
    const addBizAttrs = !!(payload.secureMetaServiceLabel || payload.secureMetaServiceLabels)
    await relayMessage(sock, remoteJid, msg.message, {
        messageId: msg.key.id,
        additionalAttributes: options.additionalAttributes,
        additionalNodes: nodes,
        recipientOverrides: options.recipientOverrides,
        specificRecipient: options.specificRecipient,
        addBizAttributes: addBizAttrs || undefined
    })
    return msg
}
...
export async function sendProduct(sock, remoteJid, product, quoted = null, options = {}) {
    ...
    if (buttons.length) {
        productMessage.nativeFlowMessage = {
            buttons,
            messageParamsJson: ''
        }
    }
    ...
    const addBizAttrs = !!(product.secureMetaServiceLabel || product.secureMetaServiceLabels)
    await relayMessage(sock, remoteJid, msg.message, {
        messageId: msg.key.id,
        additionalAttributes: options.additionalAttributes,
        additionalNodes: options.additionalNodes,
