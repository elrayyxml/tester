...
export function createMessageApi(ctx) {
    ...
    /**
     * Event message (messages.md event → eventMessage).
     * Signature: sendEvent(jid, event, quoted?) or sendEvent(jid, quoted, event)
     */
    async function sendEvent(remoteJid, eventOrQuoted, maybeEvent, options = {}) {
        let event = eventOrQuoted;
        let quoted = null;
        let opts = options || {};

        // sendEvent(jid, quoted, event)
        if (
            maybeEvent &&
            typeof maybeEvent === 'object' &&
            (maybeEvent.startDate || maybeEvent.name) &&
            eventOrQuoted &&
            typeof eventOrQuoted === 'object' &&
            eventOrQuoted.key
        ) {
            quoted = eventOrQuoted;
            event = maybeEvent;
        } else if (
            eventOrQuoted &&
            typeof eventOrQuoted === 'object' &&
            eventOrQuoted.key &&
            !eventOrQuoted.startDate &&
            maybeEvent
        ) {
            quoted = eventOrQuoted;
            event = maybeEvent;
        } else if (maybeEvent && typeof maybeEvent === 'object' && !maybeEvent.startDate && !maybeEvent.name) {
            opts = maybeEvent || {};
            quoted = null;
        } else if (maybeEvent && typeof maybeEvent === 'object' && maybeEvent.key) {
            quoted = maybeEvent;
        }
        ...
        if (event.call && typeof opts.getCallLink === 'function') {
            const token = await opts.getCallLink(event.call, { startTime });
            const prefix = event.call === 'audio' ? 'https://call.whatsapp.com/voice/' : 'https://call.whatsapp.com/video/';
            eventMessage.joinLink = prefix + token;
        } else if (event.call && typeof sock.getCallLink === 'function') {
            const token = await sock.getCallLink(event.call, { startTime });
            const prefix = event.call === 'audio' ? 'https://call.whatsapp.com/voice/' : 'https://call.whatsapp.com/video/';
            eventMessage.joinLink = prefix + token;
        }
        ...
        // Prefer generateWAMessage { event } when engine supports it
        try {
            return await relayMessagePipeline(
                ctx,
                remoteJid,
                { event },
                {
        ...
        await caps.relayMessage(
            ...
            relayOpts(
                ctx,
                { ...opts, messageId: fullMsg.key?.id || messageId, event: true },
                fullMsg.message
            )
        );
        return fullMsg;
