...
export function createMessageApi(ctx) {
     ...
     * Send a sticker.
     * Options:
     * - looked: true → anti-colong (avatar/AI EXIF + isAvatar/isAiSticker)
     * - premium: 1|true → premium sticker EXIF
     * - isLottie / isAvatar / packname / author / emojis
     */
    ...
    /**
     * Send a sticker pack.
     * Builds stickerPackMessage (JSZip + engine.encryptedStream + waUploadToServer),
     * then relays via generateWAMessageFromContent — does not rely on the
    ...
    async function sendStickerPack(remoteJid, pack, quoted = null, options = {}) {
        ...
        const stickers = pack.stickers.map((item, index) => {
            const source = item?.sticker ?? item?.data ?? item?.media ?? item;
            if (
                source == null ||
                ...
                throw createError(Err.INVALID_MEDIA, `Invalid sticker at index ${index}`);
            }
            return {
                sticker: source,
        ...
        if (typeof upload !== 'function') {
            throw createError(Err.INVALID_ENGINE, 'waUploadToServer is required for sticker packs');
        }

        let stickerPackMessage;

        // Prefer native Baileys prepare when the stickers content branch exists
        if (typeof caps.prepareStickerPackMessage === 'function') {
            stickerPackMessage = await caps.prepareStickerPackMessage(
                {
                    stickers: stickers.map((s) => ({
                        data: toMediaSource(s.sticker),
                        emojis: s.emojis,
                        accessibilityLabel: s.accessibilityLabel
                    })),
                    cover: toMediaSource(pack.cover),
                ...
                },
                {
                    ...
                    mediaCache: opts.mediaCache,
                    options: opts.httpOptions || opts.options,
                    mediaUploadTimeoutMs: opts.mediaUploadTimeoutMs
                }
            );
        } else {
            try {
                stickerPackMessage = await buildStickerPackMessage(
                    {
                        name: pack.name || pack.packname || '',
                        publisher: pack.publisher || pack.author || '',
                        description: pack.description || pack.desc || '',
                ...
                // Last resort: stickers content key (Lia@ forks with generateWAMessageContent support)
                if (err?.message?.includes('encryptedStream')) {
                    return relayMessagePipeline(
                        ctx,
                        remoteJid,
                        {
