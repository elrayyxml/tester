...
export function createMessageApi(ctx) {
    ...
    async function sendEvent(remoteJid, event, quoted = null, options = {}) {
        ...
        const content = {
            eventMessage,
            messageContextInfo: {
                messageSecret: event.messageSecret || undefined
            }
        };
        ...
        await caps.relayMessage(
            remoteJid,
            fullMsg.message,
            relayOpts(ctx, { ...opts, messageId: fullMsg.key?.id || messageId, event: true }, fullMsg.message)
        );
        return fullMsg;
    }
    ...
    async function sendPoll(remoteJid, values, pollOptions = {}, quoted = null, options = {}) {
        ...
        content.messageContextInfo = {
            messageSecret: pollOptions.messageSecret || undefined
        };
        ...
        await caps.relayMessage(
            ...
            relayOpts(
                ctx,
                {
                    ...
                    polltype: pollOptions.pollType === 1 ? 'quiz_creation' : 'creation'
                },
                fullMsg.message
            )
        );
    ...
    async function sendPollResult(remoteJid, name, votes, quoted = null, options = {}) {
        ...
        let content;
        if (opts.pollType === 1 || options?.pollType === 1) {
            pollResultSnapshotMessage.pollType =
                caps.proto?.Message?.PollType?.QUIZ ?? 1;
        ...
        await caps.relayMessage(
            ...
            relayOpts(
                ctx,
                { ...opts, messageId: fullMsg.key?.id || messageId, polltype: 'creation' },
                fullMsg.message
            )
        );
        return fullMsg;
