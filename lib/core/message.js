...
export function makeMessageId(context, options = {}) {
    ...
    if (typeof config.custom_id === 'string' && config.custom_id.length > 0) {
        return `${config.custom_id}-${getRandom(18)}`
    }
    if (config.stealth) {
 ...
 *
 * `mentionsAll` does not resolve participant JIDs. Instead it marks the
 * message with `mentionAll` so the engine sets
 * `contextInfo.nonJidMentions = 1` (mention-all without a JID list).
 *
 * @param {object} sock - The augmented engine socket.
...
export async function resolveMentions(sock, remoteJid, options = {}) {
    const mentionAll = options.mentionsAll === true || options.mentionAll === true
 ...
 * @param {boolean} [options.mentionsAll=false] - Mention all group participants.
 * @param {string[]} [options.mentions] - Mentioned JIDs.
 ...
 * @param {object|null} [quoted=null] - Optional quoted message.
 * @param {object} [options={}] - Message options.
 * @param {boolean} [options.ptv=false] - Send as a PTV (video note).
...
export async function sendVideo(sock, remoteJid, video, caption = '', quoted = null, options = {}) {
    ...
    const msg = await generateMessage(sock, remoteJid, { videoMessage }, {
        quoted,
        messageId: options.messageId,
        ephemeralExpiration: options.expiration,
        engineOptions: options.engineOptions
...
export async function sendLocation(sock, remoteJid, location, quoted = null, button = null, options = {}) {
    ...
    if (buttons.length > 0) {
        content = {
            interactiveMessage: {
                body: { text: caption || locationMessage.name || 'Location' },
                ...(options.footer ? { footer: { text: options.footer } } : {}),
                nativeFlowMessage: { buttons },
                header: { hasMediaAttachment: false, locationMessage }
            }
        }
 ...
 * @param {boolean} [options.withExif=true] - Embed EXIF metadata when packname/author provided.
 ...
 * @param {number|boolean} [options.premium] - Premium sticker flag.
...
export async function sendSticker(sock, remoteJid, sticker, quoted = null, options = {}) {
    ...
    const hasMetadata = options.packname || options.author || options.metadata
    if (hasMetadata && options.withExif !== false) {
    ...
    const msg = await generateMessage(sock, remoteJid, { stickerMessage }, {
        quoted,
        messageId: options.messageId,
        engineOptions: options.engineOptions
    })
 ...
 * @param {object} sock - Initialized client socket.
 * @param {string} remoteJid - Destination JID.
 * @param {import('../types/utils.js').StickerPackPayload} pack - Sticker pack payload.
 * @param {object|null} [quoted=null] - Optional quoted message.
 * @param {object} [options={}] - Message options.
 * @returns {Promise<object>} Generated WAMessage.
 */
export async function sendStickerPack(sock, remoteJid, pack, quoted = null, options = {}) {
    assertJid(remoteJid)
    quoted = resolveQuoted(quoted, options)
    if (!pack || !Array.isArray(pack.stickers)) {
...
export async function buildInteractiveMessage(sock, payload = {}) {
    ...
    const buttons = payload.interactiveButtons || payload.buttons || []
    if (!Array.isArray(buttons)) {
        throw createError('interactiveButtons must be an array.', ErrorCodes.INVALID_OPTIONS)
    }
...
export function buildVCard(contact) {
    ...
    const bizName = contact.bizName || contact.businessName
    if (bizName) {
        lines.push(`X-WA-BIZ-NAME:${bizName}`)
    }
    const bizDescription = contact.bizDescription ?? contact.businessDescription
 ...
 * @param {number} [product.priceAmount1000] - Price in 1000ths.
...
export async function sendProduct(sock, remoteJid, product, quoted = null, options = {}) {
    ...
    const content = {
        ...
        hasMediaAttachment: product.hasMediaAttachment
    }
    if (product.interactiveButtons || product.buttons) {
        content.interactiveButtons = product.interactiveButtons || product.buttons
    }
 ...
 * @param {object} sock - Initialized client socket.
 * @param {string} remoteJid - Destination JID.
 * @param {import('../types/baileys.js').MediaInput} image - Live photo image.
 * @param {import('../types/baileys.js').MediaInput} video - Live photo video.
 * @param {object|null} [quoted=null] - Optional quoted message.
 * @param {object} [options={}] - Message options.
 * @param {string} [options.caption] - Optional caption.
 * @returns {Promise<object>} Generated WAMessage (the image message).
 */
export async function sendLivePhoto(sock, remoteJid, image, video, quoted = null, options = {}) {
    assertJid(remoteJid)
    quoted = resolveQuoted(quoted, options)
    const context = getContext(sock)
    ...
    const msg = await generateMessage(sock, remoteJid, { imageMessage, videoMessage }, {
        quoted,
        messageId: options.messageId,
        engineOptions: options.engineOptions
    })
...
export async function sendCard(sock, remoteJid, payload, quoted = null, options = {}) {
    ...
    for (const card of payload.cards) {
        const cardMedia = card.image
            ? { imageMessage: await prepareMedia(sock, 'image', card.image, {
                jid: remoteJid,
                engineOptions: options.engineOptions
            }) }
            : card.video
                ? { videoMessage: await prepareMedia(sock, 'video', card.video, {
                    jid: remoteJid,
                    engineOptions: options.engineOptions
                }) }
                : {}
        cards.push({
            cardId: card.cardId || `card_${cards.length + 1}`,
            title: card.title || '',
    ...
    if (Object.keys(contextInfo).length) {
        carouselMessage.contextInfo = contextInfo
    }
    const content = { interactiveMessage: { carouselMessage } }
    const msg = await generateMessage(sock, remoteJid, content, {
        quoted,
        messageId: options.messageId,
        engineOptions: options.engineOptions
 ...
 * @param {import('../types/baileys.js').PollOptions|object|null} [options=null] - Poll options or quoted message (legacy form).
 * @param {object} [quotedOrOptions={}] - Quoted message or options.
 * @returns {Promise<object>} Generated WAMessage.
 */
export async function sendPoll(sock, remoteJid, name, values, options = null, quotedOrOptions = {}) {
    assertJid(remoteJid)
    ...
    const pollOptionsPayload = values.map((option, index) => ({
        optionName: option,
    ...
    const msg = await generateMessage(sock, remoteJid, {
        pollCreationMessage: {
            name,
            options: pollOptionsPayload,
            selectableOptionsCount: pollOptions.selectableOptionsCount || 1,
            messageSecret: makeMessageSecret()
        }
    }, {
        quoted,
...
/**
 * Sends a quiz (poll with a single correct answer).
 *
 * @param {object} sock - Initialized client socket.
 * @param {string} remoteJid - Destination JID.
 * @param {string} name - Quiz question.
 * @param {string[]} values - Quiz options.
 * @param {number} correctOption - Index of the correct option (1-based).
 * @param {object|null} [quoted=null] - Optional quoted message.
 * @param {object} [options={}] - Message options.
 * @returns {Promise<object>} Generated WAMessage.
 */
export async function sendQuiz(sock, remoteJid, name, values, correctOption, quoted = null, options = {}) {
    assertJid(remoteJid)
    if (typeof correctOption !== 'number' || correctOption < 1 || correctOption > values.length) {
    ...
    const msg = await generateMessage(sock, remoteJid, {
        pollCreationMessage: {
            ...
            messageSecret: makeMessageSecret(),
            optionsVersion: String(correctOption)
        }
    }, {
        quoted,
        messageId: options.messageId,
    ...
    const nodes = normalizeAdditionalNodes(options.additionalNodes).concat(NODES.quiz_creation)
    await relayMessage(sock, remoteJid, msg.message, {
        messageId: msg.key.id,
        additionalAttributes: options.additionalAttributes,
        additionalNodes: nodes
...
export async function sendEvent(sock, remoteJid, event, quoted = null, options = {}) {
    ...
    const endTime = normalizeDate(event.endDate ?? event.endTime, 'endDate')
    ...
    if (event.call) {
        eventMessage.call = event.call
    }
    if (event.isCancelled !== undefined) {
        eventMessage.isCancelled = event.isCancelled
    }
    ...
    const msg = await generateMessage(sock, remoteJid, { eventMessage }, {
        quoted,
        messageId: options.messageId,
        engineOptions: options.engineOptions
    })
 ...
 * @param {string} invoice.currencyCodeIso4217 - Currency code (ISO 4217).
