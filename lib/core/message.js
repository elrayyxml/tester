...
import { buildAdditionalNodes, interactiveBizNode } from './node.js';
...
export async function relayMessagePipeline(ctx, jid, content, options = {}) {
    ...
    let payload = content;
    if (useSecure && payload && typeof payload === 'object') {
    ...
    await caps.relayMessage(jid, fullMsg.message, {
        ...
        addBizAttributes: useSecure || opts.addBizAttributes,
...
export function createMessageApi(ctx) {
    ...
    async function sendLivePhoto(remoteJid, video, quoted = null, options = {}) {
        ...
        await caps.relayMessage(remoteJid, parent.message, { messageId: parent.key.id });
    ...
    /**
     * Carousel (messages.md: cards → interactiveMessage.carouselMessage).
     * Media per card: image | video | product (Buffer|path|url).
     * Buttons: buttons | nativeFlow → nativeFlowMessage.
     */
    async function sendCard(remoteJid, pack, quoted = null, options = {}) {
        ...
        const cards = pack.cards.map((card, i) => {
            if (!card || typeof card !== 'object') {
                throw createError(Err.INVALID_MESSAGE, `Invalid card at ${i}`);
            }
            if (card.image == null && card.video == null && card.product == null) {
                throw createError(Err.INVALID_MEDIA, `Card ${i} needs image, video, or product`);
            }
            ...
            const entry = {
                ...
                audioFooter: card.audioFooter
            };

            if (card.image != null) entry.image = toMediaSource(card.image);
            if (card.video != null) entry.video = toMediaSource(card.video);
            if (card.product != null) entry.product = card.product;

            const buttons = card.buttons || card.nativeFlow || card.interactiveButtons;
            if (buttons) entry.nativeFlow = buttons;

            return entry;
        });

        const content = { cards, ...contentFlags(opts) };
        if (pack.text != null) content.text = String(pack.text);
        if (pack.footer != null) content.footer = String(pack.footer);

        return relayMessagePipeline(ctx, remoteJid, content, {
            ...opts,
            quoted,
            ...
            contextInfo: opts.contextInfo,
            messageId: opts.messageId,
            interactive: opts.nativeFlowName || 'mixed',
            additionalNodes: opts.additionalNodes
        });
    }

    /**
     * Interactive + nativeFlow (messages.md: interactiveButtons | nativeFlow).
     * Header media: image | video | location.
     * Body: text | caption. Footer: footer | audioFooter.
     *
     * await sock.sendInteractive(jid, {
     *   image|video|location, text|caption, footer, title, subtitle,
     *   mentions, interactiveButtons|buttons|nativeFlow
     * }, quoted?)
     */
    async function sendInteractive(remoteJid, payload, quoted = null, options = {}) {
        ...
        // Prefer interactiveButtons key (messages.md escape hatch); nativeFlow also works
        const content = Array.isArray(buttons)
            ? { interactiveButtons: buttons, ...contentFlags(opts) }
            : { interactiveButtons: buttons, ...contentFlags(opts) };

        if (payload.image != null) content.image = toMediaSource(payload.image);
        if (payload.video != null) content.video = toMediaSource(payload.video);
        if (payload.location != null) content.location = payload.location;

        if (payload.text != null) content.text = String(payload.text);
        else if (payload.caption != null) content.caption = String(payload.caption);

        if (payload.footer != null) content.footer = String(payload.footer);
        if (payload.audioFooter != null) content.audioFooter = payload.audioFooter;

        if (payload.title != null) content.title = String(payload.title);
        if (payload.subtitle != null) content.subtitle = String(payload.subtitle);
        if (payload.header != null) content.header = payload.header;
        if (payload.thumbnail != null) content.thumbnail = payload.thumbnail;

        if (payload.messageParamsJson != null) content.messageParamsJson = payload.messageParamsJson;
        if (payload.paramsJson != null) content.paramsJson = payload.paramsJson;
        if (payload.bizJid != null) content.bizJid = payload.bizJid;
        if (payload.shopSurface != null) content.shopSurface = payload.shopSurface;
        if (payload.id != null) content.id = payload.id;
        ...
        return relayMessagePipeline(ctx, remoteJid, content, {
            ...
            contextInfo: opts.contextInfo,
            messageId: opts.messageId,
            interactive: opts.nativeFlowName || payload.nativeFlowName || 'mixed',
            additionalNodes: opts.additionalNodes
        });
    }
