 ...
 *
 * When a prefix is provided it is prepended to the device-specific ID,
 * producing e.g. `NEXRAY3A...` for iOS with a custom prefix.
 *
 * @param {string} device - Device identifier (ios, android, web, desktop).
 * @param {string} [prefix=''] - Optional prefix prepended to the device ID.
 * @returns {string} Message ID shaped like the target device.
 */
export function generateStealthId(device, prefix = '') {
    const hex = (n) => randomBytes(Math.ceil(n / 2)).toString('hex').toUpperCase().slice(0, n)
    switch (device) {
        case 'ios':
            return '3A' + hex(18)
        case 'web':
 ...
 *
 * Priority: explicit `messageId` option, then `custom_id` prefix (combined
 * with stealth device format when stealth is configured), then stealth
 * device format alone, then a plain hex ID.
 *
...
export function makeMessageId(context, options = {}) {
    ...
    if (config.stealth) {
        return generateStealthId(config.stealth, config.custom_id || '')
    }
...
export async function prepareMedia(sock, mediaType, mediaInput, options = {}) {
    ...
    if (annotation && (mediaType === 'image' || mediaType === 'video' || mediaType === 'document' || mediaType === 'audio')) {
    ...
    if (annotation && (mediaType === 'image' || mediaType === 'video' || mediaType === 'document' || mediaType === 'audio')) {
        if (!preparedMessage.annotations || !preparedMessage.annotations.length) {
            preparedMessage.annotations = [annotation]
        }
        if (!preparedMessage.contextInfo) {
            preparedMessage.contextInfo = {}
        }
        preparedMessage.contextInfo.forwardedNewsletterMessageInfo = {
            newsletterJid: annotation.newsletter.newsletterJid,
            newsletterName: annotation.newsletter.newsletterName,
            serverMessageId: annotation.newsletter.serverMessageId ?? Math.floor(Math.random() * 999999),
            contentType: annotation.newsletter.contentType
        }
        preparedMessage.contextInfo.accessibilityText = annotation.newsletter.accessibilityText
    }
...
export async function generateMessage(sock, jid, content, options = {}) {
    ...
    const needsTransform = !hasMessageContentKey(content) && typeof transform === 'function'
    if (needsTransform) {
        ...
        if (transformed && hasMessageContentKey(transformed)) {
            payload = transformed
        }
    }
...
export async function relayMessage(sock, jid, message, options = {}) {
    ...
    if (options.addBizAttributes !== undefined) {
        relayOptions.addBizAttributes = options.addBizAttributes
    } else if (options.secureMetaServiceLabel || options.secureMetaServiceLabels || context.config?.metaLabel) {
        relayOptions.addBizAttributes = true
    }
...
export async function sendText(sock, remoteJid, text, quoted = null, options = {}) {
    ...
    await relayMessage(sock, remoteJid, msg.message, {
        ...
        secureMetaServiceLabels: options.secureMetaServiceLabels
    })
    return msg
}
 ...
 * @returns {Promise<object>} Generated WAMessage.
 */
export async function sendStickerPack(sock, remoteJid, payload, quoted = null) {
    assertJid(remoteJid)
    quoted = resolveQuoted(quoted, payload)

    if (!payload || typeof payload !== 'object') {
        throw createError('sendStickerPack requires a payload object.', ErrorCodes.INVALID_MESSAGE)
    }
    ...
    const stickerItems = stickers.map((s) => ({
        media: s.sticker || s.media || s,
        emojis: s.emojis || payload.emojis,
        accessibilityLabel: s.accessibilityLabel
    }))

    const stickerPayload = await buildStickerPackPayload({
    ...
    const msg = await generateMessage(sock, remoteJid, stickerPayload, {
        quoted,
    ...
    await relayMessage(sock, remoteJid, msg.message, {
        ...
        specificRecipient: payload.specificRecipient,
        secureMetaServiceLabel: payload.secureMetaServiceLabel,
        secureMetaServiceLabels: payload.secureMetaServiceLabels
    })
    return msg
}
...
export async function buildInteractiveMessage(sock, payload = {}) {
    const context = getContext(sock)
    const annotation = resolveNewsletterAnnotation(context, payload, payload.jid)
    ...
    if (annotation) {
        ...
        contextInfo.accessibilityText = annotation.newsletter.accessibilityText
    }
...
export async function sendInteractive(sock, remoteJid, payloadOrButtons, quotedOrOptions, maybeOptions) {
    ...
    await relayMessage(sock, remoteJid, msg.message, {
        ...
        recipientOverrides: options.recipientOverrides,
        specificRecipient: options.specificRecipient,
        secureMetaServiceLabel: payload.secureMetaServiceLabel || options.secureMetaServiceLabel,
        secureMetaServiceLabels: payload.secureMetaServiceLabels || options.secureMetaServiceLabels
    })
    return msg
}
...
export async function sendProduct(sock, remoteJid, product, quoted = null, options = {}) {
    ...
    const context = getContext(sock)
    const annotation = resolveNewsletterAnnotation(context, options, remoteJid)

    const buttons = normalizeFlowButtons(product.interactiveButtons || product.buttons || [])
    ...
    if (annotation) {
        if (!interactiveMessage.contextInfo) {
            interactiveMessage.contextInfo = {}
        }
        interactiveMessage.contextInfo.forwardedNewsletterMessageInfo = {
            newsletterJid: annotation.newsletter.newsletterJid,
            newsletterName: annotation.newsletter.newsletterName,
            serverMessageId: annotation.newsletter.serverMessageId ?? Math.floor(Math.random() * 999999),
            contentType: annotation.newsletter.contentType
        }
        interactiveMessage.contextInfo.accessibilityText = annotation.newsletter.accessibilityText
    }
    ...
    const existingNodes = Array.isArray(options.additionalNodes)
        ? options.additionalNodes
        : options.additionalNodes ? [options.additionalNodes] : []
    await relayMessage(sock, remoteJid, msg.message, {
        messageId: msg.key.id,
        additionalAttributes: options.additionalAttributes,
        additionalNodes: [...existingNodes, ...NODES.catalog_message],
        secureMetaServiceLabel: product.secureMetaServiceLabel || options.secureMetaServiceLabel,
        secureMetaServiceLabels: product.secureMetaServiceLabels || options.secureMetaServiceLabels
    })
    return msg
}
    ...
    if (event.call && !joinLink) {
        ...
        if (typeof getCallLink === 'function') {
            try {
                ...
                // Fallback: leave joinLink empty when getCallLink fails
            }
        }
    }
