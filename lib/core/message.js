...
export function createMessageApi(ctx) {
    ...
    /**
     * Event message (messages.md eventMessage).
    ...
    async function sendEvent(remoteJid, event, quoted = null, options = {}) {
        ...
        if (event.endDate != null) {
            ...
            if (!Number.isNaN(endDate.getTime())) endTime = endDate.getTime() / 1000;
        }
        ...
        await caps.relayMessage(
            ...
            relayOpts(
                ctx,
                {
                    ...
                    event: true,
                    additionalNodes: [...(opts.additionalNodes || []), eventNode]
                },
                fullMsg.message
            )
        );
    ...
    /**
     * Poll creation (messages.md poll → pollCreationMessage / V3).
    ...
    async function sendPoll(remoteJid, values, pollOptions = {}, quoted = null, options = {}) {
        ...
        let content;
        if (pollOptions.toAnnouncementGroup) {
            content = { pollCreationMessageV2: pollCreationMessage };
        } else if (pollOptions.pollType === 1) {
        ...
        // messages.md — messageSecret always required for poll encryption key
        content.messageContextInfo = {
            messageSecret: pollOptions.messageSecret || randomBytes(32)
        };
        ...
        // messages-send.md — meta polltype node (required or poll is invisible)
        ...
        const pollNode = pollMetaNode(polltype, {
            contenttype: isNewsletter ? 'text' : undefined
        });
        ...
        await caps.relayMessage(
            ...
            relayOpts(
                ctx,
                {
                    ...
                    messageId: fullMsg.key?.id || messageId,
                    polltype,
                    additionalNodes: [...(opts.additionalNodes || []), pollNode]
                },
                fullMsg.message
            )
        );
    ...
    /**
     * Poll result snapshot (messages.md pollResult).
    ...
    async function sendPollResult(remoteJid, name, votes, quoted = null, options = {}) {
        ...
        let content;
        if (opts.pollType === 1) {
            pollResultSnapshotMessage.pollType = caps.proto?.Message?.PollType?.QUIZ ?? 1;
            content = { pollResultSnapshotMessageV3: pollResultSnapshotMessage };
        } else {
            pollResultSnapshotMessage.pollType = caps.proto?.Message?.PollType?.POLL ?? 0;
            content = { pollResultSnapshotMessage };
        }
        ...
        // poll result still uses creation meta (messages-send hypothesis)
