 ...
 *
 * @param {string} [prefix=''] - Hex prefix for the ID.
 ...
 *
 * Produces an all-uppercase hex string: [prefix] + 32 random hex chars.
 * When a `customPrefix` is provided it is prepended; otherwise the
 * library-wide default prefix is used.
 *
 * @param {string} [userJid] - Unused in V2 — kept for call-site compat.
 * @param {string} [customPrefix] - Optional prefix prepended to the hex body.
 * @returns {string} Message ID (uppercase hex).
 */
export function generateMessageIDV2(userJid, customPrefix) {
    const hex = randomBytes(16).toString('hex').toUpperCase()
    if (customPrefix) {
        return String(customPrefix).toUpperCase() + hex
    }
    return 'NEXRAY' + hex
}