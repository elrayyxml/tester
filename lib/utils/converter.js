...
export async function resolveMedia(input, options = {}) {
    ...
    switch (detection.kind) {
        ...
        case 'object':
            ...
            if (detection.value && typeof detection.value === 'object' && Buffer.isBuffer(detection.value.buffer)) {
                return {
                    buffer: detection.value.buffer,
                    mimetype: options.mimetype || detection.value.mimetype || 'application/octet-stream',
                    extension: options.extension || detection.value.extension || null,
                    isUrl: false,
                    isPath: false,
                    isBuffer: true
                }
