...
/**
 * Normalize quoted for generateWAMessage / generateWAMessageFromContent.
 * Baileys expects: { key: { remoteJid, fromMe, id, participant? }, message: IMessage }
 */
function normalizeQuoted(quoted) {
  if (!quoted) return undefined

  // Nested options.quoted
  if (quoted.quoted && !quoted.key) return normalizeQuoted(quoted.quoted)

  var key = quoted.key
  var message = quoted.message

  // Serialized m may put content at top-level fields; recover message if missing
  if (key && !message) {
    if (quoted.fakeObj && quoted.fakeObj.message) message = quoted.fakeObj.message
  }

  if (!key) return undefined

  // Ensure key has required fields as plain object (not class instance)
  var normKey = {
    remoteJid: key.remoteJid,
    fromMe: !!key.fromMe,
    id: key.id
  }
  if (key.participant) normKey.participant = key.participant
  if (key.participantAlt) normKey.participantAlt = key.participantAlt

  // Fallback message so stanzaId still attaches (conversation stub)
  if (!message) {
