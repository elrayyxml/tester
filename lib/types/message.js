...
/**
 * messages.md — interactiveButtons | nativeFlowMessage merged wrapper.
 * Escape hatch for pre-built native flow button objects; payload keys preserved.
 */
async function buildInteractiveMessage(message, { caps, upload, opts = {}, config } = {}) {
    ...
    if (!buttonsField.length) throw new globalThis.Error('interactiveButtons required');
    ...
    if (message.text != null) interactiveMessage.body = { text: String(message.text) };
    else if (message.caption != null) interactiveMessage.body = { text: String(message.caption) };

    if (message.product || message.productId || (message.businessOwnerJid && message.image)) {
        const { productMessage } = await prepareProductMessage(message, { caps, upload, opts, config });
        interactiveMessage.header = {
            title: message.header?.title || message.title || '',
            subtitle: message.header?.subtitle || message.subtitle || '',
            hasMediaAttachment: message.hasMediaAttachment !== false,
            productMessage
        };
    } else {
        const headerMedia = await prepareHeaderMedia(message, caps, upload, opts);
        const hasLocation = !!message.location;
        const locationProbe = hasLocation ? { locationMessage: {} } : {};
        if (headerMedia || hasLocation || message.title != null || message.header) {
            interactiveMessage.header = {
                title: message.header?.title || message.title || '',
                subtitle: message.header?.subtitle || message.subtitle || '',
    ...
    if (message.thumbnail) interactiveMessage.jpegThumbnail = message.thumbnail;
    if (message.footer != null) interactiveMessage.footer = { text: String(message.footer) };
