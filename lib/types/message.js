...
async function prepareMedia(input, caps, upload, opts = {}) {
    ...
    for (const key of ['image', 'video', 'audio', 'document', 'sticker']) {
        ...
        // keep string URL / Buffer as-is (Baileys getStream); only resolve local paths
        if (typeof v === 'string' && !/^https?:\/\//i.test(v) && !v.startsWith('data:')) {
            resolved[key] = await resolveMediaBuffer(v);
        } else if (Buffer.isBuffer(v) || typeof v === 'string') {
            resolved[key] = v;
        } else {
            resolved[key] = await resolveMediaBuffer(v);
        }
...
/**
 * messages.md prepareProductMessage — image via `message.image` or `message.product.productImage`.
 * Merges flat product fields (title, productId, currencyCode, …) then returns `{ productMessage }`.
...
async function prepareProductMessage(message, { caps, upload, opts = {}, config } = {}) {
    ...
    const imageSrc = message.image ?? message.product?.productImage;
    if (imageSrc == null) {
        throw createError(Err.INVALID_MEDIA, 'product image is required');
    }

    // Baileys accepts string URL/path, Buffer, or { url } — avoid wrapping twice
    let image = imageSrc;
    if (typeof imageSrc === 'string' || Buffer.isBuffer(imageSrc)) {
        image = imageSrc;
    } else if (imageSrc && typeof imageSrc === 'object' && (imageSrc.url || imageSrc.stream)) {
        image = imageSrc;
    } else {
        image = await resolveMediaBuffer(imageSrc);
    }
    ...
    const { image: _drop, ...rest } = message;
    const product = {
        ...
        salePriceAmount1000: message.salePriceAmount1000,
        retailerId: message.retailerId,
        url: message.url,
        productImageCount: message.productImageCount ?? 1,
        ...(message.product || {}),
    ...
    const body =
        message.body ||
        (message.caption != null ? { text: String(message.caption) } : undefined) ||
        (message.text != null ? { text: String(message.text) } : undefined);
    const footer =
        message.footer == null
            ? undefined
    ...
    annotateMedia(wrapped, config, caps?.proto);
    return wrapped;
}
