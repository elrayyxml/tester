...
async function prepareMedia(input, caps, upload, opts = {}) {
    ...
    for (const key of ['image', 'video', 'audio', 'document', 'sticker']) {
        if (resolved[key] != null) {
            resolved[key] = await resolveMediaBuffer(resolved[key]);
        }
    }
...
/**
 * Build productMessage — merges top-level product fields into `product` (messages.md prepareProductMessage).
 * Accepts flat keys: title, productId, currencyCode, priceAmount1000, productImageCount, image, …
...
/**
 * Normalize quoted for Baileys generateWAMessageFromContent (messages.md: `options.quoted`).
 * Accepts WebMessageInfo, serialized msg with `.key`, or nested shapes.
 * Ensures `key.fromMe` is always boolean so Baileys never throws.
 * @param {*} quoted
...
export function normalizeQuoted(quoted) {
    ...
    // nested wrappers: { message: WAMessage } / { msg: ... }
    let msg = quoted;
    if (!msg.key && msg.message?.key) msg = msg.message;
    else if (!msg.key && msg.msg?.key) msg = msg.msg;

    const key = msg.key;
    if (!key || typeof key !== 'object' || key.id == null) return undefined;
