...
export function createMessageApi(ctx) {
    ...
    async function sendText(remoteJid, text, quoted = null, options = {}) {
        ...
        if (text && typeof text === 'object') {
            return send(remoteJid, text, quoted, opts);
        }
    ...
    async function sendMedia(kind, remoteJid, media, caption, quoted, options) {
        ...
        return relayMessagePipeline(ctx, remoteJid, payload, {
            ...opts,
            quoted: normalizeQuoted(quoted) ?? opts.quoted,
            mentions: opts.mentions,
            mentionAll: opts.mentionAll,
            contextInfo: opts.contextInfo,
            externalAdReply: opts.externalAdReply,
     ...
     * Result is proto content → generateWAMessageFromContent.
    ...
    async function genWAMessageContent(message, options = {}) {
        ...
        if (hasNonNullishProperty(message, 'product') || hasNonNullishProperty(message, 'productId')) {
            // messages.md: m.productMessage = await prepareProductMessage(message, options)
            m.productMessage = await prepareProductMessage(
                {
                    ...message,
                    product: {
     ...
     * proto content → generateWAMessageFromContent; high-level → generateWAMessage.
     * metaLabel:true → secureMetaServiceLabel + biz on every message.
     *
     * @param {string} remoteJid
     * @param {object} contentOrFull
     * @param {object|null} [quoted]
     * @param {object} [options]
     */
    async function genRelay(remoteJid, contentOrFull, quoted = null, options = {}) {
        ...
        const quotedNorm = normalizeQuoted(q ?? opts.quoted);
        const genBase = {
            logger: opts.logger,
            userJid: ctx.meId,
            messageId,
    ...
    /** alias used by existing helpers */
    const relay = genRelay;
    const sendContent = genRelay;
    const nextId = (opts = {}) =>
        opts.messageId ||
        generateMessageId({ meId: ctx.meId, customId: config.customId, stealth: config.stealth });
    ...
    const withMeta = (opts = {}) => {
        const o = { ...opts };
        if (useMetaLabel(o, config)) {
            o.secureMetaServiceLabel = true;
            o.addBizAttributes = true;
        }
        return o;
    };
