...
export function createMessageApi(ctx) {
    ...
    async function sendContent(remoteJid, content, quoted, opts = {}) {
        ...
        return relay(remoteJid, fullMsg, withMeta(opts));
    }

    /**
     * Shared generateWAMessage options — quoted matches messages-send.md `options.quoted`.
     * @param {object|null} quoted
     * @param {object} [opts]
     */
    ...
    /** Merge per-call opts with client metaLabel → secureMetaServiceLabel for every helper. */
    function withMeta(opts = {}) {
        const o = { ...opts };
    ...
    async function sendProduct(remoteJid, payload, quoted = null, options = {}) {
        ...
        // Prefer FromContent builders (stable) over generateWAMessage for product/interactive
