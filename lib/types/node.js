...
/* ── hoisted biz-node constants (generic-utils.md style) ───────── */
...
export function getBizBinaryNode(message = {}) {
    ...
    if (message.listMessage) {
        // PRODUCT_LIST (2/3) vs SINGLE_SELECT (1) — messages.md / client traffic
        const listType = message.listMessage.listType;
        const listNode =
            listType === 2 || listType === 3
                ? LIST_TYPE_CONTENT
                : { tag: 'list', attrs: { v: '2', type: 'single_select' } };
        return normalizeNode({
            tag: 'biz',
            attrs: bizAttributes,
            content: [listNode, qualityContent]
        });
    }
