...
function attachAIRich(sock) {
    ...
    sock.sendMetaMsg = function (jid, parts, quoted, opts) {
        return Promise.resolve().then(function () {
            ...
            var quoted = (0, context_1.buildQuoted)(quoted);
            // Inject quoted into innermost content if present
            if (quoted && payload.botForwardedMessage && payload.botForwardedMessage.message &&
                payload.botForwardedMessage.message.richResponseMessage) {
                var rr = payload.botForwardedMessage.message.richResponseMessage;
                rr.contextInfo = Object.assign({}, rr.contextInfo || {}, {
                    stanzaId: quoted.key && quoted.key.id,
                    participant: quoted.participant || (quoted.key && quoted.key.participant),
                    quotedMessage: quoted.message
                });
            }
            return sock.relayMessage(jid, payload, {
                messageId: messageId,
                additionalNodes: options.additionalNodes || nodes_1.NODES.bot_ai
            }).then(function () {
                return {
