...
/**
 * Resolve baileys module: per-socket engines[0] / baileys override wins,
 * otherwise the module-level require. Throws only if neither is available.
 */
function getBaileys(sock) {
    var cfg = sock && sock.__nexray;
    var resolved = (cfg && ((Array.isArray(cfg.engines) && cfg.engines[0]) || cfg.baileys)) || baileys;
    if (!resolved) {
...
function attachSendHelpers(sock) {
    ...
    function sendText(jid, text, quoted, opts) {
        return __awaiter(this, void 0, void 0, function () {
            var options, mentions, groupMeta, contextInfo, content, msg, messageId;
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        if (!jid)
                        ...
                        if (!(options.mentionAll && jid.endsWith('@g.us') && typeof sock.groupMetadata === 'function')) return [3 /*break*/, 2];
                        return [4 /*yield*/, sock.groupMetadata(jid).catch(function () { return null; })];
                    case 1:
                        groupMeta = _a.sent();
                        if (groupMeta && Array.isArray(groupMeta.participants)) {
                            mentions = groupMeta.participants.map(function (p) { return p.id; });
                        }
                        _a.label = 2;
                    case 2:
    ...
    function prepareStickerBuffer(input, opts) {
        return __awaiter(this, void 0, void 0, function () {
            ...
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0: return [4 /*yield*/, resolveToBuffer(normalizeMediaInput(input))];
                    case 1:
                        buf = _a.sent();
                        if (!buf) {
    ...
    function buildPollCreation(name, values, options, pollType) {
        ...
        var payload = {
            ...
            options: values.map(function (v) { return { optionName: String(v) }; }),
            hideParticipantName: !!options.hideVoter,
            allowAddOption: !!options.canAddOption
        };
        if (hasNonNullishProperty(options, 'endDate')) {
            payload.endTime = new Date(options.endDate).getTime();
        }
        var key = 'pollCreationMessage';
        if (options.toAnnouncementGroup) {
            key = 'pollCreationMessageV2';
        }
        else if (pollType === 1) {
            ...
            payload.correctAnswer = { optionName: String(options.correctAnswer) };
            payload.pollType = 1;
            payload.selectableOptionsCount = 1;
            key = 'pollCreationMessageV5';
        }
        else if (selectableCount === 1) {
            key = 'pollCreationMessageV3';
        }
        return { key: key, value: payload };
    }

    /**
     * Shared poll/quiz send path — builds the raw proto message and relays it.
     * Does not depend on baileys' generateWAMessageContent supporting 'poll' at all.
     */
    function sendPollCreationRaw(jid, name, values, options, quoted, pollType) {
        return __awaiter(this, void 0, void 0, function () {
            var q, built, content, messageId, msg;
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        q = resolveQuoted(quoted, options);
                        built = buildPollCreation(name, values, options, pollType);
                        messageId = options.messageId || makeMsgId(sock);
                        content = {};
                        content[built.key] = built.value;
                        content.messageContextInfo = { messageSecret: require('crypto').randomBytes(32) };
    ...
    function sendPoll(jid, name, optsOrValues, quoted) {
        ...
        return sendPollCreationRaw(jid, options.name, values, options, quoted, 0);
    }
    ...
    function sendQuiz(jid, values, quoted, opts) {
        ...
        return sendPollCreationRaw(jid, options.name || '', values, options, quoted, 1);
    }

    /**
     * Build a pollResultSnapshotMessage* proto object directly.
     */
    function buildPollResultSnapshot(name, votes, isQuiz) {
        if (!Array.isArray(votes) || !votes.length) {
            throw new errors_1.NexrayError('poll result votes must be a non-empty array', errors_1.ErrorCodes.INVALID_OPTIONS);
        }
        var pollVotes = votes.map(function (v) { return ({ optionName: String(v.name), optionVoteCount: parseInt(v.voteCount, 10) || 0 }); });
        var pollTypeEnum = (proto && proto.Message && proto.Message.PollType) || { POLL: 0, QUIZ: 1 };
        var payload = {
            name: name,
            pollVotes: pollVotes,
            pollType: isQuiz ? pollTypeEnum.QUIZ : pollTypeEnum.POLL
        };
        return { key: isQuiz ? 'pollResultSnapshotMessageV3' : 'pollResultSnapshotMessage', value: payload };
    }

    /**
     * Shared poll-result/quiz-result send path — raw proto, no baileys 'pollResult' dependency.
     */
    function sendPollResultRaw(jid, name, votes, quoted, opts, isQuiz) {
        return __awaiter(this, void 0, void 0, function () {
            var options, q, built, content, messageId, msg;
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        options = opts || {};
                        q = resolveQuoted(quoted, options);
                        built = buildPollResultSnapshot(name, votes, isQuiz);
                        messageId = options.messageId || makeMsgId(sock);
                        content = {};
                        content[built.key] = built.value;
                        msg = generateWAMessageFromContent(jid, content, {
    ...
    function sendPollResult(jid, name, votes, quoted, opts) {
        return sendPollResultRaw(jid, name, votes, quoted, opts, false);
    }

    /**
     * sendQuizResult(jid, name, votes, message?, opts?) — same shape as sendPollResult,
     * tagged QUIZ so WA renders the trophy/quiz result card.
     *   sendQuizResult(jid, '🏆 Quiz Result', [{ name: '🔥 Fire', voteCount: 133 }], m)
     */
    function sendQuizResult(jid, name, votes, quoted, opts) {
        return sendPollResultRaw(jid, name, votes, quoted, opts, true);
    }
     ...
     * non-empty, plausibly-JPEG result instead of trusting a single 00:00:00 attempt.
     */
    ...
    async function sendLivePhoto(jid, media, quoted, opts) {
        ...
        if (hasNonNullishProperty(media, 'image')) {
            ...
            var extractVideoThumb = baileys.extractVideoThumb;
            if (!extractVideoThumb) {
                throw new errors_1.NexrayError('sendLivePhoto: baileys.extractVideoThumb unavailable and no image was provided — pass { image } explicitly', errors_1.ErrorCodes.NOT_IMPLEMENTED);
            }
            var fs = require('fs');
            ...
            if (Buffer.isBuffer(media.video)) {
                ...
                // remote url — download to a temp file first so ffmpeg can actually seek it
            ...
            var frameBuf = inputPath ? await extractLiveThumb(extractVideoThumb, inputPath) : null;
            if (needCleanup && inputPath) {
                try { fs.unlinkSync(inputPath); } catch (_ignored) { /* best-effort cleanup */ }
            }
            if (!frameBuf) {
                throw new errors_1.NexrayError('sendLivePhoto: could not extract a valid frame from the video — pass { image } explicitly instead', errors_1.ErrorCodes.MEDIA_PROCESS);
            }
            imageInput = frameBuf;
        }
            ...
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        ...
                        if (!(typeof input === 'string')) return [2 /*return*/, null];
                        if (!/^https?:\/\//i.test(input)) return [3 /*break*/, 5];
                        _a.label = 1;
                    case 1:
                        _a.trys.push([1, 4, , 5]);
                    ...
                    case 2:
                        ...
                        if (!res.ok) {
                            throw new errors_1.NexrayError(
                                (label || 'resolveToBuffer') + ': fetch failed for "' + input + '" — HTTP ' + res.status + ' ' + res.statusText,
                    ...
                    case 4:
                        ...
                        throw new errors_1.NexrayError(
                            (label || 'resolveToBuffer') + ': failed to fetch "' + input + '" — ' + (err_1 && err_1.message),
                            errors_1.ErrorCodes.MEDIA_DOWNLOAD
                        );
                    case 5:
                        fs = require('fs');
                        if (!fs.existsSync(input)) {
                            throw new errors_1.NexrayError((label || 'resolveToBuffer') + ': local file not found — ' + input, errors_1.ErrorCodes.INVALID_MEDIA);
                        }
                        return [2 /*return*/, fs.readFileSync(input)];
                }
            });
        });
    }
