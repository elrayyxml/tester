...
/**
 * Peer dependency "baileys" — required once at module load, never per-call.
...
/**
 * Resolve baileys module: per-socket override (sock.__nexray.baileys) wins,
 * otherwise the module-level require. Throws only if neither is available.
 */
function getBaileys(sock) {
    var resolved = (sock && sock.__nexray && sock.__nexray.baileys) || baileys;
    if (!resolved) {
...
function attachSendHelpers(sock) {
    ...
    sock.sendSticker = function (jid, sticker, quoted, opts) {
        return __awaiter(this, void 0, void 0, function () {
            var options, q, messageId, mediaInput, prepared, stickerMessage, msgContent, msg, isPremium, isAi, isAvatar, isLocked, isLottie;
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        options = opts || {};
                        ...
                        messageId = options.messageId || makeMsgId(sock);
                        mediaInput = normalizeMediaInput(sticker);
                        return [4 /*yield*/, baileys.prepareWAMessageMedia
                            ? baileys.prepareWAMessageMedia({ sticker: mediaInput }, { upload: sock.waUploadToServer })
                            : Promise.resolve(null)];
                    case 1:
                        prepared = _a.sent();
                        if (prepared && prepared.stickerMessage) {
                            stickerMessage = Object.assign({}, prepared.stickerMessage);
                        }
                        else {
                            // fallback will use generateWAMessage below
                            stickerMessage = null;
                        }
                        if (stickerMessage) {
                            ...
                            if (isPremium || options.premium != null)
                                stickerMessage.premium = typeof options.premium === 'number' ? options.premium : 1;
                            if (options.emojis)
                                stickerMessage.emojis = Array.isArray(options.emojis) ? options.emojis.join('') : String(options.emojis);
                    ...
                    case 2:
                        _a.sent();
                        return [2 /*return*/, msg];
                    case 3: return [4 /*yield*/, generateWAMessage(jid, { sticker: mediaInput }, {
                            userJid: sock.user && sock.user.id,
                            quoted: (0, context_1.buildQuoted)(q) || undefined,
                            messageId: messageId,
                            upload: sock.waUploadToServer
                    ...
                    case 4:
                        ...
                        if (msg.message && msg.message.stickerMessage) {
                            if (isAvatar)
                                msg.message.stickerMessage.isAvatar = true;
                            if (isAi)
                                msg.message.stickerMessage.isAiSticker = true;
                            if (isLottie)
                                msg.message.stickerMessage.isLottie = true;
                            if (options.accessibilityLabel)
                                msg.message.stickerMessage.accessibilityLabel = options.accessibilityLabel;
                            if (isPremium)
                                msg.message.stickerMessage.premium = 1;
                            if (options.emojis)
                                msg.message.stickerMessage.emojis = Array.isArray(options.emojis) ? options.emojis.join('') : String(options.emojis);
                        }
                        if (isLottie && msg.message && msg.message.stickerMessage) {
                            msg.message = {
                                lottieStickerMessage: { message: { stickerMessage: msg.message.stickerMessage } }
                            };
                        }
                        if (isLocked || isPremium) {
    ...
    /**
     * sendPoll(jid, name, options?, quoted?)
    ...
    sock.sendPoll = function (jid, name, optsOrValues, quoted) {
        return __awaiter(this, void 0, void 0, function () {
            var options, values, q, contextInfo, messageId, msg;
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        options = {};
                        values = [];
                        // sendPoll(jid, [values], m, { name, ... }) — baileys-native positional shape
                        if (Array.isArray(name)) {
                            values = name;
                            options = quoted || {};
                            q = resolveQuoted(optsOrValues, options);
                            name = options.name || '';
                        }
                        else if (Array.isArray(optsOrValues)) {
                            values = optsOrValues;
                            options = (quoted && typeof quoted === 'object' && quoted.key == null) ? quoted : {};
                            q = resolveQuoted(quoted, options);
                        }
                        else if (optsOrValues && typeof optsOrValues === 'object') {
                            options = optsOrValues;
                            values = options.options || options.values || [];
                            q = resolveQuoted(quoted || options.quoted, options);
                        }
                        q = q || resolveQuoted(quoted, options);
                        contextInfo = (0, context_1.buildContextInfo)({
                            quoted: q,
                            mentions: options.mentions || options.mentionedJid,
                            extra: options.contextInfo
                        ...
                        messageId = options.messageId || makeMsgId(sock);
                        return [4 /*yield*/, generateWAMessage(jid, {
                                poll: buildPollCreation(name, values, options, 0),
                                contextInfo: Object.keys(contextInfo).length ? contextInfo : undefined
                            }, {
                                userJid: sock.user && sock.user.id,
                                quoted: (0, context_1.buildQuoted)(q) || undefined,
    ...
    /**
     * sendQuiz(jid, values, quoted?, options)
     * Newsletter-only. options: { name, correctAnswer, endDate?, hideVoter?, canAddOption? }
     *   sock.sendQuiz('123@newsletter', ['Yes','No'], m, { name: 'Quiz!', correctAnswer: 'Yes' })
     */
    sock.sendQuiz = function (jid, values, quoted, opts) {
        return __awaiter(this, void 0, void 0, function () {
            var options, q, name, contextInfo, messageId, msg;
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        options = opts || {};
                        ...
                        messageId = options.messageId || makeMsgId(sock);
                        return [4 /*yield*/, generateWAMessage(jid, {
                                poll: buildPollCreation(name, values, options, 1),
                                contextInfo: Object.keys(contextInfo).length ? contextInfo : undefined
                            }, {
                                userJid: sock.user && sock.user.id,
                                quoted: (0, context_1.buildQuoted)(q) || undefined,
    ...
    /**
     * Shared poll-result-snapshot builder — used by sendPollResult and sendQuizResult.
     * votes: [{ name, voteCount }]
     */
    function buildPollResultSnapshot(name, votes) {
        if (!Array.isArray(votes)) {
            throw new errors_1.NexrayError('poll result votes must be an array', errors_1.ErrorCodes.INVALID_OPTIONS);
        }
        return {
            name: name,
    ...
    /**
     * sendPollResult(jid, name, votes, quoted?, opts?)
     *   sock.sendPollResult(jid, '📈 Poll Result', [{ name: '🔥 Fire', voteCount: 133 }], m)
     */
    sock.sendPollResult = function (jid, name, votes, quoted, opts) {
        return __awaiter(this, void 0, void 0, function () {
            ...
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        ...
                        messageId = options.messageId || makeMsgId(sock);
                        return [4 /*yield*/, generateWAMessage(jid, {
                                pollResult: buildPollResultSnapshot(name, votes),
                                contextInfo: Object.keys(contextInfo).length ? contextInfo : undefined
                            }, {
                                userJid: sock.user && sock.user.id,
                                quoted: (0, context_1.buildQuoted)(q) || undefined,
    ...
    /**
     * sendQuizResult(jid, name, votes, quoted?, opts?) — same shape as sendPollResult,
     * but tags the snapshot as pollType QUIZ so WA renders the trophy/quiz result card.
     *   sock.sendQuizResult(jid, '🏆 Quiz Result', [{ name: '🔥 Fire', voteCount: 133 }], m)
     */
    sock.sendQuizResult = function (jid, name, votes, quoted, opts) {
        return __awaiter(this, void 0, void 0, function () {
            var options, q, contextInfo, messageId, snapshot, msg;
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        options = opts || {};
                        ...
                        messageId = options.messageId || makeMsgId(sock);
                        snapshot = buildPollResultSnapshot(name, votes);
                        snapshot.pollType = 1;
                        return [4 /*yield*/, generateWAMessage(jid, {
                                pollResult: snapshot,
                                contextInfo: Object.keys(contextInfo).length ? contextInfo : undefined
    ...
    /**
     * pollResult(jid, { name, votes: [{ name, count }] }, quoted?, opts?) — neoxr-compatible alias.
     * Wraps sendPollResult, translating neoxr's { votes: [{name, count}] } payload shape
     * into the { name, voteCount } shape sendPollResult/baileys expect.
     */
    sock.pollResult = function (jid, payload, quoted, opts) {
        payload = payload || {};
        var votes = (payload.votes || []).map(function (v) {
            return { name: v.name, voteCount: v.voteCount != null ? v.voteCount : v.count };
        });
        return sock.sendPollResult(jid, payload.name || '', votes, quoted, opts);
    };
     ...
     */
    sock.sendProduct = function (jid, productOrPayload, quoted, opts) {
        return __awaiter(this, void 0, void 0, function () {
     ...
     */
    sock.groupStatus = function (jid, content, opts) {
        return __awaiter(this, void 0, void 0, function () {
    ...
    sock.sendStatusMentions = function (jids, content, opts) {
        return __awaiter(this, void 0, void 0, function () {
     ...
     * Baileys' extractVideoThumb seeks with ffmpeg `-ss <time>` then grabs 1 frame.
     * Seeking to the exact start (00:00:00) frequently lands before the first
     * keyframe on many encodes, so ffmpeg returns an empty/corrupt buffer.
     * Fix: try a short list of small forward offsets and keep the first
     * non-empty result instead of trusting a single 00:00:00 attempt.
     */
    function extractLiveThumb(extractVideoThumb, inputPath) {
        return __awaiter(this, void 0, void 0, function () {
            var offsets, i, buf;
            return __generator(this, function (_a) {
                switch (_a.label) {
     ...
     */
    sock.sendLivePhoto = function (jid, media, quoted, opts) {
        return __awaiter(this, void 0, void 0, function () {
            var options, q, imageInput, vidPrepared, imgPrepared, imgMsg, vidContent, frameBuf, extractVideoThumb, fs, pathMod, os, inputPath, needCleanup, isBuf;
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        options = opts || {};
                        ...
                        if (hasNonNullishProperty(media, 'image')) return [3 /*break*/, 2];
                        extractVideoThumb = baileys.extractVideoThumb;
                        if (!extractVideoThumb) return [3 /*break*/, 2];
                        fs = require('fs');
                        pathMod = require('path');
                        os = require('os');
                        isBuf = Buffer.isBuffer(media.video);
                        needCleanup = false;
                        if (isBuf) {
                            ...
                            // remote url — prepareMedia will upload it directly; no local file to seek
                            inputPath = null;
                        }
                        if (!inputPath) return [3 /*break*/, 2];
                        return [4 /*yield*/, extractLiveThumb(extractVideoThumb, inputPath)];
                    case 1:
                        frameBuf = _a.sent();
                        if (needCleanup)
                            try { fs.unlinkSync(inputPath); } catch (_b) { }
                        if (frameBuf)
                            imageInput = frameBuf;
                        _a.label = 2;
                    case 2:
                        if (!imageInput)
                            imageInput = media.video; // last resort — WA will still accept a video-derived preview
                        return [4 /*yield*/, prepareMedia(imageInput, 'image', options)];
                    case 3:
                        imgPrepared = _a.sent();
                        return [4 /*yield*/, prepareMedia(media.video, 'video', options)];
                    case 4:
                        vidPrepared = _a.sent();
                        imgMsg = generateWAMessageFromContent(jid, {
                        ...
                        return [4 /*yield*/, relay(jid, imgMsg.message, { messageId: imgMsg.key.id })];
                    case 5:
                        _a.sent();
                        vidContent = {
                        ...
                        return [4 /*yield*/, relay(jid, vidContent, { messageId: makeMsgId(sock) })];
                    case 6:
                        _a.sent();
                        return [2 /*return*/, imgMsg];
                }
            });
        });
    };
    ...
    function resolveToBuffer(input) {
        return __awaiter(this, void 0, void 0, function () {
            var fs, streamFn, chunks, stream, chunk;
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        ...
                        if (!/^https?:\/\//i.test(input)) return [3 /*break*/, 3];
                        // baileys getStream if available
                        streamFn = baileys.getStream;
                        if (!(typeof streamFn === 'function')) return [3 /*break*/, 2];
                        return [4 /*yield*/, streamFn({ url: input })];
                    case 1:
                        stream = _a.sent();
                        if (stream && stream.stream) {
                            chunks = [];
                            return [4 /*yield*/, new Promise(function (resolve, reject) {
                                    stream.stream.on('data', function (c) { chunks.push(c); });
                                    stream.stream.on('end', function () { resolve(Buffer.concat(chunks)); });
                    ...
                    case 2:
                        // fallback fetch
                        return [4 /*yield*/, fetch(input).then(function (r) { return r.arrayBuffer(); }).then(function (ab) { return Buffer.from(ab); }).catch(function () { return null; })];
                    case 3:
                        if (typeof input === 'string') {
                            fs = require('fs');
                            if (fs.existsSync(input)) return [2 /*return*/, fs.readFileSync(input)];
                        }
                        return [2 /*return*/, null];
                    case 4: return [2 /*return*/, null];
                }
     ...
     */
    sock.sendThumbnailPreview = function (jid, text, opts, message) {
        return __awaiter(this, void 0, void 0, function () {
            var options, q, title, body, largeThumb, ratio, url, duration, postType, thumbBuffer, iconBuffer, linkPreview, dims, imageMsg, prepared, content, msg, messageId;
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        options = opts || {};
                        if (opts && typeof opts === 'object' && (opts.key || opts.message) && message == null) {
                            message = opts;
                            options = {};
                        }
                        q = resolveQuoted(message || options.quoted, options);
                        title = options.title || '';
                        body = options.body || options.description || '';
                        largeThumb = !!options.largeThumb;
                        ratio = options.ratio || 'landscape';
                        url = options.url || '';
                        duration = options.duration || 0;
                        postType = options.postType != null ? options.postType : 1;
                        messageId = options.messageId || makeMsgId(sock);
                        return [4 /*yield*/, resolveToBuffer(options.thumbnail)];
                    case 1:
                        thumbBuffer = _a.sent();
                        linkPreview = {
                            'matched-text': url,
                        ...
                        if (thumbBuffer)
                            linkPreview.jpegThumbnail = thumbBuffer;
                        if (!largeThumb) return [3 /*break*/, 5];
                        if (!thumbBuffer) {
                            throw new errors_1.NexrayError('thumbnail required when largeThumb: true', errors_1.ErrorCodes.INVALID_MEDIA);
                        }
                        return [4 /*yield*/, baileys.prepareWAMessageMedia({ image: thumbBuffer }, {
                                upload: sock.waUploadToServer,
                                mediaTypeOverride: 'thumbnail-link'
                            })];
                    case 2:
                        prepared = _a.sent();
                        imageMsg = prepared.imageMessage;
                        dims = previewDimension(ratio);
                        if (imageMsg) {
                        ...
                        return [4 /*yield*/, resolveToBuffer(options.icon)];
                    case 3:
                        iconBuffer = _a.sent();
                        content = { text: text, linkPreview: linkPreview };
                        if (iconBuffer)
                            content.favicon = iconBuffer;
                        return [4 /*yield*/, generateWAMessage(jid, content, {
                                userJid: sock.user && sock.user.id,
                                quoted: (0, context_1.buildQuoted)(q) || undefined,
                                messageId: messageId,
                    ...
                    case 5:
                        ...
                        return [4 /*yield*/, generateWAMessage(jid, { text: text, linkPreview: linkPreview }, {
                                userJid: sock.user && sock.user.id,
                                quoted: (0, context_1.buildQuoted)(q) || undefined,
                                messageId: messageId,
                                upload: sock.waUploadToServer
