...
function attachSendHelpers(sock) {
    ...
    sock.sendSticker = function (jid, sticker, quoted, opts) {
        return __awaiter(this, void 0, void 0, function () {
            var options, q, contextInfo, messageId, mediaInput, prepared, stickerMessage, msgContent, msg, isPremium, isAi, isAvatar, isLocked;
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        options = opts || {};
                        ...
                        // Prefer prepareWAMessageMedia for sticker upload
                        if (!baileys.prepareWAMessageMedia) return [3 /*break*/, 3];
                        return [4 /*yield*/, baileys.prepareWAMessageMedia({ sticker: mediaInput }, { upload: sock.waUploadToServer })];
                    case 1:
                        prepared = _a.sent();
                    ...
                    case 2:
                        ...
                        return [2 /*return*/, msg];
                    case 3:
                        // fallback generateWAMessage
                        contextInfo = (0, context_1.buildContextInfo)({ quoted: q, extra: options.contextInfo });
                        return [4 /*yield*/, generateWAMessage(jid, {
                                sticker: mediaInput,
                                contextInfo: Object.keys(contextInfo).length ? contextInfo : undefined
                            }, {
            ...
            return __generator(this, function (_a) {
                switch (_a.label) {
                    ...
                    case 1:
                        ...
                        additionalNodes = options.additionalNodes || raw.additionalNodes || [{
                                tag: 'biz',
                                attrs: {},
                                content: [{
                                        tag: 'interactive',
     ...
     * Neoxr-style group/status mention.
     * content: { media, caption } | { text, background?, color? } | { media, background } for audio
     * opts.private?: { name, emoji }
     */

    /**
     * groupStatus(jid, content, opts?)
     * content:
     *   { media, caption } | { image|video|audio, caption }
     *   { text, background?, font?, color? }
     * opts:
     *   private?: { name?, emoji? }  // status privacy label (neoxr-style)
     *   background?: color hex for text status
     *   font?: number
     */
    sock.groupStatus = function (jid, content, opts) {
        return __awaiter(this, void 0, void 0, function () {
            var options, STORIES_JID, payload, msg, statusJidList, meta, participants, i, p, type, mentionMsg, targets;
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        options = opts || {};
                        ...
                        payload = {};
                        if (content.media || content.image || content.video || content.audio) {
                            if (content.video || (content.media && (options.type === 'video' || content.type === 'video')))
                                payload.video = normalizeMediaInput(content.video || content.media);
                            else if (content.audio || content.background && content.media)
                                payload.audio = normalizeMediaInput(content.audio || content.media);
                            else
                            ...
                            if (content.background || options.background)
                                payload.backgroundColor = content.background || options.background;
                            if (content.font != null || options.font != null)
                                payload.font = content.font != null ? content.font : options.font;
                            if (content.color)
                                payload.textArgb = content.color;
                        }
                        else {
                            throw new errors_1.NexrayError('groupStatus requires media or text', errors_1.ErrorCodes.INVALID_OPTIONS);
                        }
                        // privacy / emoji decoration (stored on message context when supported)
                        if (options.private && typeof options.private === 'object') {
                            payload.statusAudience = options.private;
                        }
                    ...
                    case 1:
                        msg = _a.sent();
                        // attach private emoji/name into context if present
                        if (options.private && msg.message) {
                            try {
                                var firstKey = Object.keys(msg.message).find(function (k) { return k !== 'messageContextInfo'; });
                                if (firstKey && msg.message[firstKey]) {
                                    if (!msg.message[firstKey].contextInfo)
                                        msg.message[firstKey].contextInfo = {};
                                    if (options.private.emoji)
                                        msg.message[firstKey].contextInfo.statusSourceType = options.private.emoji;
                                }
                            }
                            catch (_b) { }
                        }
                        statusJidList = [];
                        targets = Array.isArray(jid) ? jid : [jid];
                        i = 0;
                        _a.label = 2;
                    case 2:
                        if (!(i < targets.length)) return [3 /*break*/, 6];
                        if (!targets[i].endsWith('@g.us')) return [3 /*break*/, 4];
                        return [4 /*yield*/, sock.groupMetadata(targets[i])];
                    case 3:
                        meta = _a.sent();
                    ...
                    case 7:
                        _a.sent();
                        type = (typeof jid === 'string' && jid.endsWith('@g.us'))
                        ...
                        return [4 /*yield*/, sock.relayMessage(targets[0], mentionMsg, {
                                additionalNodes: [{
                                        tag: 'meta',
    ...
    /**
     * sendLivePhoto(jid, { image?, video }, quoted?, opts?)
     * If image omitted, thumbnail frame is extracted from video (ffmpeg).
     */
    sock.sendLivePhoto = function (jid, media, quoted, opts) {
        return __awaiter(this, void 0, void 0, function () {
            var options, q, imageInput, vidPrepared, imgPrepared, imgMsg, vidContent, frameBuf;
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        options = opts || {};
                        ...
                        return [4 /*yield*/, extractVideoFrame(media.video)];
                    case 1:
                        frameBuf = _a.sent();
                        imageInput = frameBuf || media.video;
                        _a.label = 2;
                    case 2: return [4 /*yield*/, prepareMedia(imageInput, 'image', options)];
                    case 3:
                        imgPrepared = _a.sent();
    ...
    /** Extract first frame of video via ffmpeg (optional). Returns Buffer or null. */
    function extractVideoFrame(videoInput) {
        return __awaiter(this, void 0, void 0, function () {
            var fs, path, os, ffmpeg, inputPath, outPath, tmp, isUrl, isBuf, needCleanup;
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        _a.trys.push([0, 2, , 3]);
                        ...
                        tmp = os.tmpdir();
                        isBuf = Buffer.isBuffer(videoInput);
                        isUrl = typeof videoInput === 'string' && /^https?:\/\//i.test(videoInput);
                        needCleanup = false;
                        if (isBuf) {
                            inputPath = path.join(tmp, 'nexray_live_' + Date.now() + '.mp4');
                        ...
                        else if (videoInput && videoInput.url) {
                            inputPath = videoInput.url;
                        }
                        else {
                            return [2 /*return*/, null];
                        }
                        outPath = path.join(tmp, 'nexray_frame_' + Date.now() + '.jpg');
                        return [4 /*yield*/, new Promise(function (resolve, reject) {
                                ffmpeg(inputPath)
                                    ...
                                    .on('error', function (e) { resolve(false); });
                            })];
                    case 1:
                        if (!_a.sent() || !fs.existsSync(outPath)) {
                            if (needCleanup)
                                try {
